...
func init() {
	cli.AddDecimalVarP(mortgageCmd.Flags(), &mf.Amount, "amount", "a", "The loan amount borrowed.")
	cli.AddDecimalVarP(mortgageCmd.Flags(), &mf.Rate, "rate", "r", "Annual interest rate.")
	mf.Years = decimal.NewFromInt(30)
	cli.AddDecimalVarP(mortgageCmd.Flags(), &mf.Years, "years", "y", "Loan term in years")

	mortgageCmd.MarkFlagRequired("amount")
	mortgageCmd.MarkFlagRequired("rate")

	// optional flags
	cli.AddDecimalVar(mortgageCmd.Flags(), &mf.ExtraMonthlyPayment, "extra-monthly", "Extra monthly payment.")
	cli.AddDecimalVar(mortgageCmd.Flags(), &mf.ExtraAnnualPayment, "extra-annual", "Extra annual payment.")
